...
public class ArticleController extends BaseController {
    ...
    @PostMapping("/doEdit")
    @ResponseBody
    public R doEdit(@Valid Article article, String tagName, BindingResult result) {
    ...
    public R appreciable(@PathVariable("id") Long id, Boolean appreciable) {
        return builder("修改赞赏状态").exec(() -> articleRepository.updateAppreciableById(appreciable, id) == 1);
    }
    ...
    @PostMapping("/post")
    @ResponseBody
    public R post(@Valid Article article, String tagName, BindingResult result) {
        if (!result.hasErrors()) {
