...
import me.wuwenbin.blog.v201801.common.TagType;
...
public class ArticleServiceImpl implements ArticleService {
    ...
    private void getSumByFilterContent(Article article) {
        if (LangUtils.string.isEmpty(article.getSummary())) {
            ...
            article.setSummary(clearContent.substring(0, clearContent.length() < 250 ? clearContent.length() : 250));
        } else {
            article.setSummary(StringUtils.trimAllWhitespace(WebUtils.filterHtml(article.getSummary())));
        }
    }
}
